...
new ProductModel(){ ProductModelID = 19, Name = "Mountain-100"  },
            ...
            var result = from p in products
                         ...
                         join pm in productmodel on p.ProductModelID equals pm.ProductModelID
                         select new 
                         {
                         p.ProductID,
                         p.Name,
                         category = pc.ProductCategoryID,
                         model= pm.ProductModelID 
                         };

            foreach (var item in result)
            {
                Console.WriteLine("Product info is : ");
                Console.WriteLine($"ProductId = {item.ProductID} \t\t ProductName= {item.Name} Category = {item.category} Model = {item.model}");
            }
            ...
            //var result = (from p in products
            //             join c in productcategory on p.ProductCategoryID equals c.ProductCategoryID
            //             join pm in productmodel on p.ProductModelID equals pm.ProductModelID
            //             select new
            //             {
            //                 p.ProductID,p.Name,p.ListPrice,p.StandardCost,
            //                 productcategory = c.Name , productmodel = pm.ProductModelID
            //             }).ToList();

            //foreach (var item in result)
            //{
            //    Console.Write("Products Info is : ");
            //    Console.WriteLine($"ProductId: {item.ProductID}\t\t ProductCategoryId : {item.Name}\t\t Model: {item.productmodel}\t\t");
            //}
